...
func (con FocusController) DoAdd(c *gin.Context) {
	...
	focusImage, err4 := models.UploadImg(c, "focus_img")
	if err4 != nil {
		fmt.Println(err4)
	}
...
func (con FocusController) Edit(c *gin.Context) {
	...
	if err1 != nil {
		con.Error(c, "传入参数失败", "admin/focus")
		return
	}
	focusList := models.Focus{Id: id}
	models.DB.Find(&focusList)
...
func (con FocusController) DoEdit(c *gin.Context) {
	...
	if err5 != nil {
		con.Error(c, "修改数据失败请重试", "/admin/focus/edit?id="+models.String(id))
	} else {
		con.Success(c, "增加轮播图成功", "/admin/focus")
	}
}
...
func (con FocusController) Delete(c *gin.Context) {
	id, err1 := models.Int(c.Query("id"))
	if err1 != nil {
		con.Error(c, "传入数据有误", "admin/focus")
		return
	}
	focusList := models.Focus{Id: id}
	models.DB.Find(&focusList)
